...
class AdminDishControllerTest extends AbstractControllerTest {
    ...
    void get() throws Exception {
        perform(MockMvcRequestBuilders
                .get(REST_URL + DISH_START_ID, BAVARIUS_ID))
    ...
    void getAllByRestaurant() throws Exception {
        perform(MockMvcRequestBuilders.get(REST_URL, BAVARIUS_ID))
                .andExpect(status().isOk())
                .andDo(print())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
                .andExpect(DISH_MATCHER.contentJson(bavarius_menu));
    }
    ...
    void update() throws Exception {
        ...
        perform(MockMvcRequestBuilders.put(REST_URL + DISH_START_ID, BAVARIUS_ID)
                ...
                .andExpect(status().isNoContent());
        DISH_MATCHER.assertMatch(dishRepository.get(START_SEQ + 11, BAVARIUS_ID).orElse(null), updated);
    }
    ...
    void createDuplicate() throws Exception {
        Dish invalid = new Dish(null, bavariusDish5.getName(), 2.0);
        perform(MockMvcRequestBuilders.post(REST_URL, BAVARIUS_ID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.writeValue(invalid)))
                .andDo(print())
    ...
    void updateDuplicate() throws Exception {
        Dish invalid = new Dish(DISH_START_ID + 4, bavariusDish5.getName(), 11.0);
        perform(MockMvcRequestBuilders.put(REST_URL + (DISH_START_ID + 4), BAVARIUS_ID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.writeValue(invalid)))
                .andDo(print())
                .andExpect(status().isUnprocessableEntity())
